...
class Player(tk.Frame):
	...
	def play_song(self, event=None):
		...
		mixer.music.play()
		samples = pygame.sndarray.samples(mixer.Sound(self.playlist[self.current]))
		sampling_rate = mixer.get_init()[0]

		self.plot_frequency_graph(samples, sampling_rate)
	def pause_song(self):
		if not self.paused:
			self.paused = True
			mixer.music.pause()
	...
	def plot_frequency_graph(self, samples, sampling_rate):
		n = len(samples)
